...
import main.de.bossascrew.pathfinder.data.files.FileRoadMap;
import main.de.bossascrew.pathfinder.system.Edge;
import main.de.bossascrew.pathfinder.system.Node;
import main.de.bossascrew.pathfinder.visualization.EdgeManager;
...
public class RoadMap {
	...
	public void delete() {
		...
		for(int i = 0; i < roadmaps.size(); i++) {
			if(roadmaps.get(i).key.equals(key)) roadmaps.remove(i);
		}
	}
	...
	public static RoadMap getRoadMap(String key) {
		for(RoadMap rm : roadmaps) {
			if(rm.key.equalsIgnoreCase(key)) return rm;
		}
		return null;
	}
...
//	}
	...
	public void removeEdge(int key1, int key2) {
		for(Node n : fileWegpunkte.waypoints) {
			...
			if(n.id == key1) {
				for(Edge e : n.adjacencies) if(e.target.id != key2) edges.add(e);
				n.adjacencies = edges.toArray(new Edge[0]);
			} else if(n.id == key2) {
				for(Edge e : n.adjacencies) if(e.target.id != key1) edges.add(e);
				n.adjacencies = edges.toArray(new Edge[0]);
			}
		}
		visualizer.refresh();
