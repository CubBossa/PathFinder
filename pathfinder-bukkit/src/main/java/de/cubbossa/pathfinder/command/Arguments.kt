...
class Arguments {
    ...
    fun location(node: String, type: LocationType):
            CommandArgument<de.cubbossa.pathfinder.misc.Location, CustomArgument<de.cubbossa.pathfinder.misc.Location, Location>> {
        return pathFinderArgument(
    ...
    fun location(node: String): CommandArgument<de.cubbossa.pathfinder.misc.Location, CustomArgument<de.cubbossa.pathfinder.misc.Location, Location>> {
        return location(node, LocationType.PRECISE_POSITION)
    }


    fun literal(literal: String): CustomLiteralArgument {
        return CustomLiteralArgument(literal)
    }
     ...
     * @return The argument
     */
    inline fun <reified E : Enum<E>> enumArgument(nodeName: String, scope: Class<E>): Argument<E> {
        return pathFinderArgument(CustomArgument(StringArgument(nodeName)) { info ->
            try {
                enumValueOf<E>(info.input().uppercase())
    ...
    fun pagination(size: Int): CommandArgument<Pagination, CustomArgument<Pagination, Int>> {
        return pathFinderArgument(CustomArgument(IntegerArgument("page", 1)) {
    ...
    fun miniMessageArgument(
        ...
        return pathFinderArgument(GreedyStringArgument(nodeName))
            .includeSuggestions { info, builder ->
                ...
                val splits = info.currentInput().split(" ", -1)
                val input = splits[splits.length - 1]
     ...
     */
    fun pathVisualizerArgument(nodeName: String): Argument<out PathVisualizer<*, *>> {
        return pathFinderArgument(
     ...
     * @return a path visualizer argument instance
     */
    fun <T : PathVisualizer<*, *>> pathVisualizerArgument(
        nodeName: String,
        type: VisualizerType<T>
    ): Argument<T> {
        return pathFinderArgument(
            customArgument(NamespacedKeyArgument(nodeName)) { customArgumentInfo ->
                PathFinder.get().getStorage()
                    .loadVisualizer<T>(BukkitPathFinder.convert(customArgumentInfo.currentInput()))
                    ?: throw CustomArgument.CustomArgumentException.fromString("There is no visualizer with this key.")
            }
        ).includeSuggestions(suggestNamespacedKeys {
    ...
    private fun suggestNamespacedKeys(supplier: suspend (CommandSender) -> Collection<NamespacedKey>): ArgumentSuggestions<CommandSender> {
        return ArgumentSuggestions { suggestionInfo, suggestionsBuilder ->
            runBlocking {
                ...
                val splits = suggestionInfo.currentInput.split(" ", -1)
                val input = splits[splits.length - 1]
     ...
     */
    fun <N : Node> nodeTypeArgument(nodeName: String): Argument<NodeType<N>> {
        return pathFinderArgument(
            customArgument(NamespacedKeyArgument(nodeName)) { customArgumentInfo ->
                val type = PathFinder.get().getNodeTypeRegistry()
                    .getType<N>(BukkitPathFinder.convert(customArgumentInfo.currentInput()))
                if (type == null) {
     ...
     */
    fun nodeSelectionArgument(nodeName: String): Argument<out NodeSelection> {
        return pathFinderArgument(
            ...
            val offset = suggestionInfo.currentInput().length - suggestionInfo.currentArg().length
     ...
     */
    fun nodeGroupArgument(nodeName: String): Argument<NodeGroup> {
        return pathFinderArgument(
            CustomArgument<NodeGroup, org.bukkit.NamespacedKey>(NamespacedKeyArgument(nodeName)) { info ->
                runBlocking {
                    PathFinder.get().getStorage()
     ...
     * @return The CustomArgument instance
     */
    fun navigateSelectionArgument(nodeName: String): Argument<NodeSelectionImpl> {
        return pathFinderArgument(
            customArgument(GreedyStringArgument(nodeName)) { context ->
                ...
                val result = HashMap<Node, Collection<SearchTermHolder>>()
                valids.forEach { result[it] = scope[it] }
     ...
     */
    fun visualizerTypeArgument(nodeName: String): Argument<VisualizerType<PathVisualizer<?, ?>>> {
        return pathFinderArgument {
            CustomArgument(NamespacedKeyArgument(nodeName)) { customArgumentInfo ->
            ...
            return CompletableFuture.completedFuture(
                PathFinder.get().getVisualizerTypeRegistry().getTypes().keySet()
            )
        }))
    }
}
