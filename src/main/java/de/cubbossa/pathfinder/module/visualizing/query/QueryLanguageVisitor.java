...
public class QueryLanguageVisitor<T extends SearchTermHolder> extends QueryLanguageBaseVisitor<Collection<T>> {
	...
	public Collection<T> visitTerm(QueryLanguageParser.TermContext context) {
		...
		return scope.stream().filter(t -> t.matches(searchTerm, attributes)).collect(Collectors.toList());
	}

	public Collection<SearchQueryAttribute> visitAnAttributeBlock(QueryLanguageParser.AttributeblockContext context) {
		return context.attributelist() == null ? new ArrayList<>() : visitAnAttributeList(context.attributelist());
	}

	public Collection<SearchQueryAttribute> visitAnAttributeList(QueryLanguageParser.AttributelistContext context) {
