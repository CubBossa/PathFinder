...
public class CompassVisualizerType extends VisualizerType<CompassVisualizer> {
	...
	public Message getInfoMessage(CompassVisualizer element) {
		return Messages.CMD_VIS_COMPASS_INFO.format(TagResolver.builder()
				...
				.resolver(Placeholder.unparsed(CompassVisualizer.PROP_RADIUS.getKey(), element.getRadius() + ""))
				.build()
		);
	}
	...
	public ArgumentTree appendEditCommand(ArgumentTree tree, int visualizerIndex, int argumentOffset) {
		return tree
				.then(subCommand("color", CustomArgs.enumArgument("value", BossBar.Color.class), BossBarVisualizer.PROP_COLOR))
				.then(subCommand("overlay", CustomArgs.enumArgument("value", BossBar.Overlay.class), BossBarVisualizer.PROP_OVERLAY))
				.then(subCommand("background", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_BACKGROUND))
				.then(subCommand("north", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_NORTH))
				.then(subCommand("east", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_EAST))
				.then(subCommand("south", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_SOUTH))
				.then(subCommand("west", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_WEST))
				.then(subCommand("target", CustomArgs.miniMessageArgument("value"), CompassVisualizer.PROP_TARGET))
	...
	public void deserialize(CompassVisualizer visualizer, Map<String, Object> values) {
		...
		// bossbar
		loadEnumProperty(values, BossBarVisualizer.PROP_COLOR.getKey(), BossBar.Color.class, visualizer::setColor);
		loadEnumProperty(values, BossBarVisualizer.PROP_OVERLAY.getKey(), BossBar.Overlay.class, visualizer::setOverlay);
		// compass
		loadProperty(values, CompassVisualizer.PROP_BACKGROUND.getKey(), String.class, visualizer::setBackgroundFormat);
		...
		loadProperty(values, CompassVisualizer.PROP_TARGET.getKey(), String.class, visualizer::setTarget);
		loadProperty(values, CompassVisualizer.PROP_RADIUS.getKey(), Integer.class, visualizer::setRadius);
	}
}
