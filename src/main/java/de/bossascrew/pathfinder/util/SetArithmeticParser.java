...
public class SetArithmeticParser<T> {
	...
	public SetArithmeticParser(Collection<T> scope, Function<T, Collection<String>> searchTermSupplier) {
		this.scope = scope;
	...
	public List<TokenMatch> tokenize(String input) {
		...
		while (index < length) {
			...
			if (currentIndex == index) {
				throw new IllegalArgumentException("Could not parse '" + input + "', containing invalid character.");
			}
		}
		return tokens;
	}
