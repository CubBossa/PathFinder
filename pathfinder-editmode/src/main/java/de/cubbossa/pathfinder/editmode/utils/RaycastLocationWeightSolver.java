...
public class RaycastLocationWeightSolver<T> implements LocationWeightSolver<T> {
  ...
  public Map<T, Double> solve(T location, Iterable<T> scope) {
    ...
    startLocation = startLocation.add(startLocation.getDirection().normalize().multiply(startLocationDirectionWeight));
    final Location fStartLocation = startLocation;

    List<Triple<Node<T>, Double, Integer>> triples = StreamSupport.stream(scope.spliterator(), false)
        .map(element -> {
          Location loc = mapper.apply(element);
          if (scopeLocationDirectionWeight != 0) {
            loc = loc.add(loc.getDirection().normalize().multiply(scopeLocationDirectionWeight));
          }
        ...
        .map(e -> new AbstractMap.SimpleEntry<>(e, e.location().distanceSquared(fStartLocation)))
        .sequential()
    ...
    triples.stream()
        ...
        .forEach(e -> result.put(e.getLeft().element(), e.getMiddle() * e.getRight() * blockCollisionWeight));
    return result;
  }
