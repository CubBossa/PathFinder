...
public class EditModeMenu {
	...
	public BottomInventoryMenu createHotbarMenu(DefaultNodeGroupEditor editor, Player editingPlayer) {
		...
		menu.setButton(1, Button.builder()
				...
				.withClickHandler(ClientNodeHandler.LEFT_CLICK_NODE, context -> {
					PathFinderAPI.get().getNode(context.getTarget()).thenAccept(node -> {
						Player player = context.getPlayer();
						NodeSelection start = new NodeSelection(context.getTarget());
						NodeSelection end = new NodeSelection(node.getEdges().stream()
								.map(Edge::getEnd)
					...
					PathFinderAPI.get().getNode(context.getTarget()).thenAccept(node -> {
						if (node instanceof Groupable<?> groupable) {
							...
							Bukkit.getScheduler().runTask(PathPlugin.getInstance(), () -> {
								PathFinderAPI.builder().withEvents().build()
										.removeNodesFromGroups(groupable.getGroups(), new NodeSelection(groupable)).thenRun(() -> {
				...
				menu.addListEntry(Button.builder()
						.withItemStack(() -> {
							ItemStack stack = new LocalizedItem.Builder(new ItemStack(
									group.hasModifier(DiscoverableModifier.class) ? Material.CHEST_MINECART : Material.FURNACE_MINECART))
						...
						.withClickHandler(Action.LEFT, c -> {
							if (!group.contains(groupable.getNodeId())) {
								Bukkit.getScheduler().runTask(PathPlugin.getInstance(), () -> {
									PathFinderAPI.get().assignNodesToGroup(group.getKey(), new NodeSelection(groupable)).thenRun(() -> {
										c.getPlayer()
						...
						.withClickHandler(Action.RIGHT, c -> {
							if (group.contains(groupable.getNodeId())) {
								...
								Bukkit.getScheduler().runTask(PathPlugin.getInstance(), () -> {
									PathFinderAPI.get().removeNodesFromGroup(group.getKey(), new NodeSelection(groupable)).thenRun(() -> {
										c.getPlayer()
